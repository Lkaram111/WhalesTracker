...
CONFIG_PATH = Path(__file__).resolve().parents[1] / "config" / "bitcoin_whales.yaml"
...
def main() -> None:
    ...
    with SessionLocal() as session:
        chain = ensure_chain(session, "bitcoin", "Bitcoin")
        inserted = 0
        for entry in whales:
            ...
            inserted += 1
        session.commit()
        print(f"Seeded {inserted} Bitcoin whales.")
